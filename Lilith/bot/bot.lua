...
function bot.kill_mob(mob_id, options)
  ...
  options.skillchain = options.skillchain or {'Savage Blade'}
  ...
  while bot.can_attack_mob(mob_id) do
    ...
    if windower.ffxi.get_player().vitals.tp >= 1000 and
      ...
      if options.heal_with_sanguine_blade and windower.ffxi.get_player().vitals.hpp < 75 then
...
function bot.use_weaponskill(name, mob_id)
  ...
  -- Switch to the requested WS target and wait only until the switch is confirmed.
  if bot.get_current_target_id() ~= mob_id then
    if not bot.engage_mob(mob_id) then
      return false
    end

    local target_switch_start = os.clock()
    while bot.get_current_target_id() ~= mob_id and
          os.clock() - target_switch_start < 2 do
      coroutine.sleep(0.05)
    end
  end

  -- Never fire the WS unless the requested target is actually selected.
  if bot.get_current_target_id() ~= mob_id then
    return false
  end
  ...
  -- Register the packet listener BEFORE sending the WS command so a fast
  -- action packet cannot be missed.
  local function ws_handler(id, data)
    if id == 0x28 then
      local action = packets.parse('incoming', data)
      if action['Actor'] ~= me_id then
        return
      ...
      if action['Category'] == 3 then  -- weaponskill
        -- Confirm this WS actually targeted the mob requested by the caller.
        if action['Target 1 ID'] == mob_id then
          success = true
          finished = true
          return
        end
      elseif action['Category'] == 4 and
             action['Target 1 Action 1 Message'] == 78 then  -- out of range
        finished = true
        return
      end

      table.insert(debug_info, 'action: ' .. tostring(action['Category']))
    ...
    elseif id == 0x29 then
      ...
      table.insert(
        debug_info,
        'action_message: ' .. tostring(action_message['Message'])
      )
    end
  end

  events.register_event('incoming chunk', ws_handler)

  -- Use <t> so GearSwap sees the text command, but only after mob_id has
  -- been confirmed as the active target above.
  windower.send_command(
    string.format('input /weaponskill "%s" <t>', name)
  )

  local start_secs = os.clock()
  local timeout = 5

  while not finished and os.clock() - start_secs < timeout do
    coroutine.sleep(0.05)
  end

  events.unregister_event('incoming chunk', ws_handler)

  if success then
    bot.forced_delay_end = os.clock() + 3
  elseif bot.log_level <= bot.log_levels.DEBUG then
    bot.debug('Weaponskill failed or timed out: ' .. tostring(name))
    for _, info in pairs(debug_info) do
      bot.debug(info)
    end
  end
...
function bot.summon_trust(party_name, spell_name)
  ...
  local retry_delay = 3
  local trust_summon_delay = 2

  for attempt = 1, max_retries do
    local party = windower.ffxi.get_party()

    if party and party.party1_count == 6 then
      return true
    end

    if bot.is_in_party(party_name) then
      return true
    end
    ...
    windower.add_to_chat(
      207,
      string.format(
        '[Lilith] Summoning %s - attempt %d/%d',
        spell_name,
        attempt,
        max_retries
      )
    ...
    windower.send_command(
      string.format('input /ma "%s" <me>', spell_name)
    )

    coroutine.sleep(5)

    if bot.is_in_party(party_name) then
      coroutine.sleep(trust_summon_delay)
      return true
    end

    coroutine.sleep(retry_delay)
  end

  windower.add_to_chat(
    123,
    string.format(
      '[Lilith] FAILED to summon %s after %d attempts.',
      spell_name,
      max_retries
    )
  )
